...
| Storage | `platform/storage` | aws-dev EBS CSI StorageClass 운영 경로 |
...
aws-dev의 공통 PVC 정책은 `platform/storage`에서 관리한다. 관측성 backend와 monitoring stack은 `medikong-aws-gp3` StorageClass를 명시해 클러스터 default StorageClass에 암묵 의존하지 않는다.
